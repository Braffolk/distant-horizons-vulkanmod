...
Status as of 2026-03-08: **Phases 1-7 + extension mod port complete.** LODs render with correct colors, depth, lightmap, transparency, compositing, SSAO, fog, and noise via a standalone extension mod (mixin-based, no DH source modifications).

## Architecture Overview

The Vulkan backend is implemented as a **standalone Fabric mod** (`dh-vulkanmod`) in the `vulkan/` subproject. It uses **mixins** to intercept DH's rendering pipeline without modifying DH's source code. This allows it to work as a drop-in extension alongside unmodified DH releases.
