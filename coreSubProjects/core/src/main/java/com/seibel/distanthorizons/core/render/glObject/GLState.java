...
public class GLState implements AutoCloseable {
	...
	public void saveState() {
		if (GLProxy.isVulkanModActive()) {
			return;
		} // Vulkan uses pipeline objects, not GL state machine
		...
		if (this.fbo != 0) {
			this.frameBufferTexture0 = GL32.glGetFramebufferAttachmentParameteri(GL32.GL_FRAMEBUFFER,
					GL32.GL_COLOR_ATTACHMENT0, GL32.GL_FRAMEBUFFER_ATTACHMENT_OBJECT_NAME);
			this.frameBufferTexture1 = GL32.glGetFramebufferAttachmentParameteri(GL32.GL_FRAMEBUFFER,
					GL32.GL_COLOR_ATTACHMENT1, GL32.GL_FRAMEBUFFER_ATTACHMENT_OBJECT_NAME);
			this.frameBufferDepthTexture = GL32.glGetFramebufferAttachmentParameteri(GL32.GL_FRAMEBUFFER,
					GL32.GL_DEPTH_ATTACHMENT, GL32.GL_FRAMEBUFFER_ATTACHMENT_OBJECT_NAME);
		} else {
			// attempting to get values from the default framebuffer can throw errors on
			// Linux
	...
	public void close() {
		if (GLProxy.isVulkanModActive()) {
			return;
		} // No GL state to restore under Vulkan

		// explicitly unbinding the frame buffer is necessary to prevent GL_CLEAR calls
		// from hitting the wrong buffer
		...
		// attempting to set textures on the default frame buffer (ID 0) will throw
		// errors
		if (frameBufferSet) {
			GL32.glFramebufferTexture2D(GL32.GL_FRAMEBUFFER, GL32.GL_COLOR_ATTACHMENT0, GL32.GL_TEXTURE_2D,
					this.frameBufferTexture0, 0);
			GL32.glFramebufferTexture2D(GL32.GL_FRAMEBUFFER, GL32.GL_COLOR_ATTACHMENT1, GL32.GL_TEXTURE_2D,
					this.frameBufferTexture1, 0);
			GL32.glFramebufferTexture2D(GL32.GL_FRAMEBUFFER, GL32.GL_DEPTH_ATTACHMENT, GL32.GL_TEXTURE_2D,
					this.frameBufferDepthTexture, 0);
		}
	...
	public String toString() {
		return "GLState{" +
				"program=" + this.program + ", vao=" + this.vao + ", vbo=" + this.vbo + ", ebo=" + this.ebo + ", fbo="
				+ this.fbo +
				", text=" + GLEnums.getString(this.texture2D) + "@" + this.activeTextureNumber + ", text0="
				...
				", blend=" + this.blend + ", scissor=" + this.scissor + ", blendMode="
				+ GLEnums.getString(this.blendSrcColor) + "," + GLEnums.getString(this.blendDstColor) +
				", depth=" + this.depth +
				", depthFunc=" + GLEnums.getString(this.depthFunc) + ", stencil=" + this.stencil +
				", stencilFunc=" + GLEnums.getString(this.stencilFunc) + ", stencilRef=" + this.stencilRef
