...
public class RenderUtil {
	...
	/**
	 * create and return a new projection matrix based on MC's modelView and
	 * projection matrices
	 *
	 * @param mcProjMat Minecraft's current projection matrix
	 */
	public static Mat4f createLodProjectionMatrix(Mat4f mcProjMat) {
		// in James' testing a near clip plane distance of 2 blocks is enough to allow
		// the fragment
		...
		if (getHeightBasedNearClipOverrideBlockDistance() == -1) {
			// min() used to prevent the near clip plane from becoming visible at large
			// vanilla render distances
			// DH's dithering/discard shader handles everything farther away anyway so the
			// near clip plane
	...
	public static float getNearClipPlaneInBlocks() {
		// Defensive cast: config system may store Double at runtime despite Float
		// generic type
		float overdraw = ((Number) Config.Client.Advanced.Graphics.Culling.overdrawPrevention.get()).floatValue();
		if (overdraw < 0) {
			// automatic mode,
	...
	private static float getNearClipPlaneDistanceInBlocks(float overdrawPreventionPercent) {
		...
		double aspectRatio = (double) MC_RENDER.getTargetFramebufferViewportWidth()
				/ MC_RENDER.getTargetFramebufferViewportHeight();

		// source:
		// https://stackoverflow.com/questions/8101119/how-do-i-methodically-choose-the-near-clip-plane-distance-for-a-perspective-proj/8101234#8101234
	...
	public static float getHeightBasedNearClipOverrideBlockDistance() {
		// always using the client level like this might cause issues with immersive
