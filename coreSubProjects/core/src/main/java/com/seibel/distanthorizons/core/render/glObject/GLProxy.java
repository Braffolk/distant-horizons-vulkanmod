...
public class GLProxy {
	...
	public static final Set<String> LOGGED_GL_MESSAGES = Collections
	...
	/** Whether VulkanMod is active (no GL context available) */
	...
	public final GLMessageBuilder vanillaDebugMessageBuilder = new GLMessageBuilder(
			(type) -> {
	...
	private GLProxy() throws IllegalStateException {
		if (VULKANMOD_ACTIVE) {
			// VulkanMod is active — no GL context available
			LOGGER.info("VulkanMod detected. Skipping OpenGL context setup — using Vulkan rendering backend.");
			...
			TIMER.scheduleAtFixedRate(TimerUtil.createTimerTask(this::manualGlCleanupTick), MS_BETWEEN_CLEANUP_TICKS,
					MS_BETWEEN_CLEANUP_TICKS);
			LOGGER.info(GLProxy.class.getSimpleName() + " creation successful (VulkanMod mode).");
			return;
		}
		...
		LOGGER.info("Creating " + GLProxy.class.getSimpleName()
				+ "... If this is the last message you see there must have been an OpenGL error.");
		...
		if (!this.glCapabilities.OpenGL32) {
			...
			MC.crashMinecraft(errorMessage,
					new UnsupportedOperationException("Distant Horizon OpenGL requirements not met"));
		}
		...
		if (Config.Client.Advanced.Debugging.OpenGl.overrideVanillaGLLogger.get()) {
			GLUtil.setupDebugMessageCallback(new PrintStream(
					new GLMessageOutputStream(GLProxy::logMessage, this.vanillaDebugMessageBuilder), true));
		}
		...
		// UNUSED currently
		// Check if we can use the named version of all calls, which is available in
		// GL4.5 or after
		...
		// Check if we can use the make-over version of Vertex Attribute, which is
		// available in GL4.3 or after
		...
		if (EPlatform.get() != EPlatform.MACOS) {
			if (vendor.contains("NVIDIA") || vendor.contains("GEFORCE")) {
				// NVIDIA card
				this.preferredUploadMethod = this.bufferStorageSupported ? EDhApiGpuUploadMethod.BUFFER_STORAGE
						: EDhApiGpuUploadMethod.SUB_DATA;
			} else {
				// AMD or Intel card
				this.preferredUploadMethod = this.bufferStorageSupported ? EDhApiGpuUploadMethod.BUFFER_STORAGE
						: EDhApiGpuUploadMethod.DATA;
			}
		} else {
		...
		LOGGER.info("GPU Vendor [" + vendor + "] with OS [" + EPlatform.get().getName()
				+ "], Preferred upload method is [" + this.preferredUploadMethod + "].");
	...
	/**
	 * @throws IllegalStateException if the Proxy hasn't been created yet and this
	 *                               is called outside the render thread
	 */
		...
		try {
			...
			LOGGER.error(
					"[" + Thread.currentThread().getName()
							+ "] ran into an unexpected error running a GL call, Error: [" + e.getMessage() + "].",
					error);
		}
	}
	...
	private static void logMessage(GLMessage msg) {
		...
		String errorMessage = "GL ERROR [" + msg.id + "] from [" + msg.source + "]: [" + msg.message + "]"
				+ (onlyLogOnce ? " this message will only be logged once" : "") + ".";
		if (onlyLogOnce
		...
		// create an exception so we get a stacktrace of where the message was triggered
		// from
