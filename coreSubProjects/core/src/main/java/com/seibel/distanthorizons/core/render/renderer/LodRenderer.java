...
public class LodRenderer {
	...
	// these ID's either what any render is currently using (since only one renderer
	// can be active at a time), or just used previously
	...
	public IVulkanRenderDelegate getVulkanDelegate() {
		return this.vulkanDelegate;
	}

	// framebuffer and texture ID's for this renderer
	private IDhApiFramebuffer framebuffer;
	/**
	 * will be null if MC's framebuffer is being used since MC already has a color
	 * texture
	 */
	@Nullable
	 ...
	 * This should only be true in the case of Optifine so LODs won't be overwritten
	...
	private void renderLodPass(RenderParams renderParams, IProfilerWrapper profiler, boolean runningDeferredPass) {
		...
		boolean useVulkan = GLProxy.isVulkanModActive() && this.vulkanDelegate != null;
		...
		if (!runningDeferredPass) {
			...
			// custom objects with SSAO (GL only)
			if (!useVulkan && Config.Client.Advanced.Graphics.GenericRendering.enableGenericRendering.get()) {
				profiler.popPush("Custom Objects");
				genericRenderer.render(renderParams, profiler, true);
			}

			// SSAO (GL only)
			if (!useVulkan && Config.Client.Advanced.Graphics.Ssao.enableSsao.get()) {
				profiler.popPush("LOD SSAO");
				SSAORenderer.INSTANCE.render(new Mat4f(renderParams.dhProjectionMatrix), renderParams.partialTicks);
			}

			// custom objects without SSAO (GL only)
			if (!useVulkan && Config.Client.Advanced.Graphics.GenericRendering.enableGenericRendering.get()) {
				profiler.popPush("Custom Objects");
			...
			// far plane clip fading (GL only)
			if (!useVulkan
					&& Config.Client.Advanced.Graphics.Quality.dhFadeFarClipPlane.get()
			...
			if (!useVulkan && Config.Client.Advanced.Debugging.DebugWireframe.enableRendering.get()) {
			...
			if (!useVulkan && this.usingMcFramebuffer) {
			...
			if (!useVulkan) {
				boolean cancelApplyShader = ApiEventInjector.INSTANCE.fireAllEvents(
	...
	private void setGLState(
		...
		if (this.nullableColorTexture != null) {
			...
			this.activeColorTextureId = GL32.glGetFramebufferAttachmentParameteri(GL32.GL_FRAMEBUFFER,
					GL32.GL_COLOR_ATTACHMENT0, GL32.GL_FRAMEBUFFER_ATTACHMENT_OBJECT_NAME);
		}

		// needs to be fired after all the textures have been created/bound
		boolean clearTextures = !ApiEventInjector.INSTANCE.fireAllEvents(DhApiBeforeTextureClearEvent.class,
				renderEventParam);
		if (clearTextures) {
			...
			if (this.usingMcFramebuffer && framebufferOverride == null) {
				...
				// otherwise we'll be writing to MC/Optifine's depth texture which causes
				// rendering issues
				framebuffer.addDepthAttachment(this.depthTexture.getTextureId(),
						EDhDepthBufferFormat.DEPTH32F.isCombinedStencil());
	...
	private boolean createRenderObjects() {
		...
		if (useVulkan) {
			// VulkanMod path: skip all GL object creation
			// Pipeline, buffers, and framebuffer are managed by VulkanRenderDelegate
			LOGGER.info("[DH-Vulkan] Skipping GL render object creation (VulkanMod active)");
			return true;
		}

		// Standard GL path
		// GLProxy should have already been created by this point, but just in case
		// create it now
		GLProxy.getInstance();
		...
		if (this.framebuffer.getStatus() != GL32.GL_FRAMEBUFFER_COMPLETE) {
			// This generally means something wasn't bound, IE missing either the color or
			// depth texture
	...
	private void createAndBindTextures() {
		...
		// DhApiColorDepthTextureCreatedEvent needs to be kept around since old versions
		// of Iris need it
		ApiEventInjector.INSTANCE.fireAllEvents(DhApiColorDepthTextureCreatedEvent.class,
				new DhApiColorDepthTextureCreatedEvent.EventParam(textureCreatedParam));
		...
		if (framebufferOverride != null) {
			framebufferOverride.addDepthAttachment(this.depthTexture.getTextureId(),
					EDhDepthBufferFormat.DEPTH32F.isCombinedStencil());
		}

		// if we are using MC's frame buffer, a color texture is already present and
		// shouldn't need to be bound
	...
	private void renderLodPass(IDhApiShaderProgram shaderProgram, RenderBufferHandler lodBufferHandler,
		...
		// GL state setup (skip under VulkanMod)
		if (!useVulkan) {
			...
			if (!opaquePass) {
				...
				GLMC.glBlendFuncSeparate(GL32.GL_SRC_ALPHA, GL32.GL_ONE_MINUS_SRC_ALPHA, GL32.GL_ONE,
		...
		if (!useVulkan && IRIS_ACCESSOR != null) {
			// done to fix a bug with Iris where face culling isn't properly set or reverted
			// in the MC state manager
			...
			if (lodBufferContainer != null) {
				for (int lodIndex = 0; lodIndex < lodBufferContainer.size(); lodIndex++) {
					...
					for (int vboIndex = 0; vboIndex < vbos.length; vboIndex++) {
						...
						if (GLProxy.isVulkanModActive() && this.vulkanDelegate != null) {
							// Vulkan draw path
							this.vulkanDelegate.drawBuffer(vbo, vbo.getVertexCount());
						} else {
							// GL draw path
							vbo.bind();
		...
		if (!useVulkan && Config.Client.Advanced.Debugging.renderWireframe.get()) {
